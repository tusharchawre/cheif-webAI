...
/**
 * Parses a given AI response object containing CheifAction XML-like structures
 * into a WebContainer FileSystemTree object.
 *
 * @param {object} aiResponseObject The AI's response object with a 'response' string field.
 * @returns {import('@webcontainer/api').FileSystemTree} The formatted FileSystemTree object.
 */
export function parseCheifArtifactToWebcontainerFiles(aiResponseObject: any) {
  const xmlContent = aiResponseObject.response;

  /** @type {import('@webcontainer/api').FileSystemTree} */
  const files = {};

  // Regex to capture the filePath and the content within each CheifAction tag.
  // s: single line mode, allows . to match newlines
  // g: global mode, finds all matches
  const regex =
    /<CheifAction type="file" filePath="([^"]+)">([\s\S]*?)<\/CheifAction>/g;
  let match;

  // Iterate over all matches found by the regex
  while ((match = regex.exec(xmlContent)) !== null) {
    const filePath = match[1]; // Captured group 1: filePath
    const fileContents = match[2].trim(); // Captured group 2: content, trimmed

    // Add the extracted file to our nested 'files' object
    addFileToTree(files, filePath, fileContents);
  }
 ...
 *
 * @param {object} tree The current level of the FileSystemTree being built.
...
// You can now use webcontainerFiles with WebContainer.mount()
