    ...
    useEffect(() => {
        async function fetchAndProcessFiles() {
            try {
                ...
                console.log("Raw AI Response XML (from /chat):", aiResponseData.response);

                const aiGeneratedFiles = parseCheifArtifactToWebcontainerFiles(aiResponseData);
                console.log("AI Generated Files (Expected additions/modifications):", aiGeneratedFiles);
    ...
    useEffect(() => {
        ...
        async function bootAndRunWebContainer() {
            try {
                ...
                lsCheck.output.pipeTo(new WritableStream({
                    write(data) { console.log('WebContainer: ls -l / output:', data); }
                }));
                await lsCheck.exit;
                ...
                installProcess.output.pipeTo(new WritableStream({
                    write(data) { console.log('WebContainer: npm install output:', data); }
                }));

                const installExitCode = await installProcess.exit;
                if (installExitCode !== 0) {
                    console.error(`WebContainer: npm install exited with code ${installExitCode}`);
                ...
                devProcess.output.pipeTo(new WritableStream({
                    write(data) { console.log('WebContainer: npm run dev output:', data); }
                }));
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
            ...
            {loading ? (
                <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', fontSize: '24px', color: '#555' }}>
                    <p>Setting up your development environment. This might take a moment...</p>
                </div>
            ) : (
                <iframe
                    width="100%"
